...
def batched_decode_kernel(
    ...
    chunk_page_ids: [padded_blocks, num_seqs] int32, row b holding each
    sequence's b-th active page. mask_by_chunk: [padded_blocks, num_seqs,
    num_kv_heads, num_queries_per_kv, block_size]. ``for_each_tile`` forms
    blocks_per_chunk-wide chunks from those
    logical block axes. rep_row_ids: [entries] int32, all query rows repeated as
    one block-major group per block slot. ``out`` None returns the result instead
    of storing it.
    """
