...
# Unlike the kernels in this package, this module reads `envs`: it picks between
# two implementations of one walk -- a process-wide path choice -- rather than
# per-call numerical configuration. Threading a bool through both paged-attention
# kernels' signatures and both backends' call sites would put a process-wide
# switch into the kernel ABI, where `dynamic=False` would specialize on it anyway.
# Read at module scope, as `backends/spyre_attn.py` reads SPYRE_ATTN_PROFILING.
...
def walk_tiles(
    ...
    *,
    ...
    Same contract as `torch_spyre`'s `for_each_tile`, and with
    SPYRE_ATTN_FOR_EACH_TILE set that is exactly what runs. Unset, the identical
    body runs under a Python `for` loop, which is what the kernels did before
    ...
    Returns:
        ...
        path stacks nothing, so it returns the last `per_tile_output` as-is; the
        paged-attention kernels return None there and read only the carry.

    Raises:
        ValueError: if the tiled operands disagree on their extent, or the extent
            is not a whole number of tiles.
    """
    ...
    # The trip count is derived from the operands rather than passed in, so both
    # paths iterate over exactly the extent `for_each_tile` would have tiled.
    extents = {
    ...
    for t in range(extent // tile_size):
        ...
        # tile the same way on both paths and there is only one body to reason about.
        tiles = tuple(
