...
def _lx_page_attn_kernel(
    ...
    """Page attention over the (page, kv_head)-folded cache.
...
class SpyreAttentionImpl(AttentionImpl[SpyreAttentionMetadata]):
    ...
    def __init__(
        ...
        if self._lx_kv_layout:
            self._attn_fn = _lx_page_attn_compiled
    ...
    def _batched_decode_preconditions_met(self, attn_metadata: "SpyreAttentionMetadata") -> bool:
        ...
        if not envs.SPYRE_BATCHED_DECODE or self._lx_kv_layout:
            return False
    ...
    def _record_one(
        ...
        if self._lx_kv_layout:
            ...
            kv_index_tables = [
                convert(heads.clone(), device=device)
                for _ in range(bucket.num_blocks)
            ]
    ...
    def _online_softmax_attention(
        ...
        for seq_idx in range(num_seqs):
            ...
            if self._lx_kv_layout:
                ...
                with _capped_attn_cores(self.num_kv_heads * padded_query_len):
                    result = _call_kernel(
                        "LX page attention",
                        self._attn_fn,
                        q_staging,
