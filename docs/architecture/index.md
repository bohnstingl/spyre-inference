...
|---|---|---|---|
| `GemmaRMSNorm` | `SpyreGemmaRMSNorm` | Spyre | A `force_compile`d `forward_native`, so the fp32 promotion is kept. Plain `RMSNorm` needs no replacement — upstream's fp32 `forward_native` lowers eagerly — but Gemma's trailing fp32 `weight` multiply does not: a STANDARD `[hidden]` operand that torch-spyre can neither broadcast against a staggered-EA activation nor de-stagger, so the kernel has to be compiled even under `enforce_eager`. |
...
`SpyreGemmaRMSNorm` passes `force_compile=True`, so it compiles as its own one-op graph
